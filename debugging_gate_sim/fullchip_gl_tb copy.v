...
// Now load previous read while requesting next
for (q=0; q<col; q=q+1) begin
  #0.5 clk = 1'b0;
  load = 1;
  if (q < col-1) begin
...
// Execute previous read while requesting next
for (q=0; q<total_cycle; q=q+1) begin
  #0.5 clk = 1'b0;
  execute = 1;
...
// Prime the FIFO: request first word
#0.5 clk = 1'b0;
ofifo_rd = 1;
pmem_wr  = 0;
#0.5 clk = 1'b1;

// Now write previous word while requesting next one
for (q=0; q<total_cycle; q=q+1) begin
  #0.5 clk = 1'b0;
  ...
  // keep reading until the last valid word has been requested
  if (q < total_cycle-1)
    ofifo_rd = 1;
  else
    ofifo_rd = 0;
